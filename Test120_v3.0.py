...
# 2018-09-25 Version 2.0
# 2018-09-27 Version 3.0 compiled for window
...
def test():
    ...
    for v in test_120:
        ...
        tid = "{}:{}".format(mn,sec)
        if(int(ans) == a*b):
            tmp = "    [{}]  Rätt!! {}x{} = {} Tid: {}".format(n,a,b,a*b,tid)
            print(tmp)
            results.append([n,1,a,b,a*b,tid])
            fileout.write(tmp+"\n")
            nb_correct += 1
        else:
            tmp = "    [{}] {}x{} != {} Fel!! Tid: {}".format(n,a,b,int(ans),tid)
            print(tmp)
            results.append([n,-1,a,b,int(ans),tid])
            fileout.write(tmp+"\n")
...
fileout = open(namn+"_"+date+".txt","w")
fileout.write(namn+"\n")
fileout.write(datum+"\n")
fileout.write("\n")
...
print("Resultat sparade i filen: " + namn + "_"+date+".txt")
